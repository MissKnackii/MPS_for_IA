...
function validate() {
  maintenanceToEdit.dateDebut = props.maintenance.dateDebut
  maintenanceToEdit.dateFin = props.maintenance.dateFin
  ...
  console.log(maintenanceToEdit)
  if (maintenanceToEdit.action === 'enregistrer') {
    if (maintenanceToEdit.commentaire) {
      if (window.confirm('Avez-vous bien pensé a justifié le report de dates dans les commentaires ?')) {
        emit('validate', maintenanceToEdit)
      }
    }else{
      window.alert('Veuillez justifier le report de dates dans les commentaires')
    }
  }else{
    emit('validate', maintenanceToEdit)
  }
    ...
    <!--<transition name="popup">-->
      <div class="modal">
        ...
        <footer class="modal-footer">
          <button id="valider" @click.prevent="validate">Terminer</button>
        </footer>
      </div>
    <!--</transition>-->
  </div>
