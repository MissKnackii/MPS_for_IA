<template>
  ...
  />
  <GammePopUp
      v-if="showGammes && this.moyens"
      @close="showGammes = false"
      :moyens="this.moyens"
  />
  <div id="filters">
    <h1>Maintenance Niveau 2</h1>
...
export default {
  ...
  methods: {
    checkAuth(){
      this.store.setDisplay('Maintenance')
      this.store.checkTokenExpiration()
      //this.store.token = ""
      axios.post(`${import.meta.env.VITE_API_URL}/auth/check-access/maintenanceNiv2`, { token: this.store.token, groups: this.store.groups})
          .then(response => {
            //console.log(response.data)
            this.store.display = 'Maintenance'
            this.$router.push('/gestion-maintenance');
          })
          .catch(error => {
            console.error(error);
            if (error.status === 401) {
