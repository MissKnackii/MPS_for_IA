  ...
  <div id="info">
    <div id="top5">
      ...
      <div id="buttons">
        ...
        <button id="ajoutRef" class="rebuts" @click="this.showPopupRef = true">AJOUT REFERENCE</button>
      </div>
  </div>
...
export default {
  ...
  methods: {
    async getTop5() {
      await fetch('http://localhost:3000/rebuts/top5', {
        method: 'GET',
      })
          .then(res => res.json())
          .then(data => {
    ...
    checkAuth(){
      this.store.setDisplay('Rebuts')
      this.store.checkTokenExpiration()
      //this.store.token = ""
      console.log(this.store.groups, this.store.token)
      axios.post(`${import.meta.env.VITE_API_URL}/auth/check-access/rebuts`, { token: this.store.token, groups: this.store.groups})
          .then(response => {
            this.$router.push('/auth-rebuts');
            this.store.display = 'Rebuts'
          })
          .catch(error => {
            console.error(error);
            if (error.status === 401) {
            ...
            if (error.status === 403) {
              ...
              this.$router.push('/gestion-rebuts');
              this.error = true;
            }
          })
    },
    async getRebuts() {
      await fetch('http://localhost:3000/rebuts/allrebuts', {
        method: 'GET',
      })
          .then(res => res.json())
          .then(data => {
            this.allData = data
            this.totalSalleBlanche = this.makeTotalSecteur(this.allData.filter((element) => element.secteur ===  'Salle blanche'))
            this.totalTri = this.makeTotalSecteur(this.allData.filter((element) => element.secteur === 'Contrôle/Assemblage/tri'))
            this.totalReprise = this.makeTotalSecteur(this.allData.filter((element) => element.secteur === 'Reprise rectification'))
            this.totalPrison = this.makeTotalSecteur(this.allData.filter((element) => element.secteur === 'prison'))
            this.resetTotal()
    ...
    async getRef() {
      await fetch('http://localhost:3000/rebuts/allref', {
        method: 'GET',
      })
          .then(res => res.json())
          .then(data => {
    ...
    async addRebut() {
      if (this.rebut.quantite < 5){
        ...
        await fetch('http://localhost:3000/rebuts/addrebuts', {
          method: 'POST',
          body: this.toSend,
        })
            .then(res => res.json())
    ...
    async addRef() {
      ...
      await fetch('http://localhost:3000/rebuts/addref', {
        method: 'POST',
        body: this.refToSend,
      })
          .then(res => res.json())
